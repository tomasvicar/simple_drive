...
class Unet(nn.Module):
    def __init__(self, filters=64,in_size=3,out_size=1,do=0,depth=5):
        ...
        if len(filters)>=3:
            self.conv2 =  nn.Sequential(unetConv2(filters[0], filters[1], dov=do ),unetConv2(filters[1], filters[1], dov=do ),unetConv2(filters[1], filters[1], dov=do ))

        if len(filters)>=4:
            self.conv3 = nn.Sequential(unetConv2(filters[1], filters[2], dov=do ),unetConv2(filters[2], filters[2], dov=do ),unetConv2(filters[2], filters[2], dov=do ))

        if len(filters)>=5:
            self.conv4 = nn.Sequential(unetConv2(filters[2], filters[3], dov=do ),unetConv2(filters[3], filters[3], dov=do ),unetConv2(filters[3], filters[3], dov=do ))


        self.center = nn.Sequential(unetConv2(filters[-2], filters[-1], dov=do ),unetConv2(filters[-1], filters[-1], dov=do ))


        # upsampling
        if len(filters)>=5:
            self.up_concat4 = unetUp(filters[4], filters[4] )
            self.up_conv4=nn.Sequential(unetConv2(filters[3]+filters[4], filters[3], dov=do ),unetConv2(filters[3], filters[3], dov=do ))

        
        if len(filters)>=4:
            self.up_concat3 = unetUp(filters[3], filters[3] )
            self.up_conv3=nn.Sequential(unetConv2(filters[2]+filters[3], filters[2], dov=do ),unetConv2(filters[2], filters[2], dov=do ))



        if len(filters)>=3:
            self.up_concat2 = unetUp(filters[2], filters[2] )
            self.up_conv2=nn.Sequential(unetConv2(filters[1]+filters[2], filters[1], dov=do ),unetConv2(filters[1], filters[1], dov=do ))


        self.up_concat1 = unetUp(filters[1], filters[1])
        self.up_conv1=nn.Sequential(unetConv2(filters[0]+filters[1], filters[0], dov=do ),unetConv2(filters[0], filters[0],do_batch=0 ))
